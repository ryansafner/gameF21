...
.pull-left[
- In order to understand expected events and mixed strategies, we’ll need to talk using probabilities

- See Handout on Blackboard for a refresher
]
...
- Recall when we discussed the idea of .hi-purple[risk dominance] in selecting among multiple Nash equilibria
...
- Suppose you (.red[Row]) suspect that .blue[Column] will play .blue[Left] with probability 0.50

- In that case, your expected payoff of playing .red[Up]:

$$\begin{align*}
E[\color{red}{Up}] &= (0.50)\color{red}{9}+(0.50)\color{red}{8}\\
&= 8.5\\
\end{align*}$$

- Your expected payoff of playing .red[Down]:

$$\begin{align*}
E[\color{red}{Down}] &= (0.50)\color{red}{10}+(0.50)\color{red}{-1000}\\
&= -495\\
\end{align*}$$

- So definitely, you would want to maximize your **expected** payoff and play .red[Up]
]
...
- What probability $p$ would .blue[Column] have to play .blue[Left] to make you **indifferent** between playing .red[Up] and .red[Down]?

$$\begin{align*}
E[\color{red}{Up}] &= E[\color{red}{Down}]\\
(p)\color{red}{9}+(1-p)\color{red}{8} & = (p)\color{red}{10}+(1-p)\color{red}{-1000}\\
9p+8-8p&= 10p-1000+1000p\\
p+8 &= 1010p - 1000\\
1008+p &= 1010p\\
1008 &= 1009p\\
p &= 0.9990\\
...
.pull-right[
.center[
![](../images/penalty_kick_game_palacios_br.png)
]
]
...
$$\begin{align*}\mathbb{E}[\color{blue}{\text{Dive Left}}] &= (p)\color{blue}{42}+(1-p)\color{blue}{7}\\
&= (0.50)\color{blue}{42}+(1-0.50)\color{blue}{7}\\
\end{align*}$$
...
$$\begin{align*}\mathbb{E}[\color{blue}{\text{Dive Right}}] &= (p)\color{blue}{5}+(1-p)\color{blue}{30}\\
&= (0.50)\color{blue}{5}+(1-0.50)\color{blue}{30}\\
\end{align*}$$
...
$$\begin{align*}
&p(\color{red}{58})+(1-p)\color{red}{93}\\
&0.50(\color{red}{58})+(1-0.50)\color{red}{93}\\
&\color{red}{75.5}
\\
\end{align*}$$
...
- Want to find the optimal probability mix that leaves your opponent(s) _indifferent_ between their strategies to respond
...
- Not always intuitive, but a simple principle to remember
]
...
ggplot(data = tibble(x = c(0,1)))+
  aes(x = x)+
  stat_function(fun = g_left, geom = "line", size = 1, color = "green")+
  stat_function(fun = g_right, geom = "line", size = 1, color = "purple")+
  geom_label(x = 0.8, y = g_left(0.8), label = "Dive Left", color = "green")+
  geom_label(x = 0.8, y = g_right(0.8), label = "Dive Right", color = "purple")+
  ...
  geom_point(x = 0.383, y = 20.41, size = 2)+
...
ggplot(data = tibble(x = c(0,1)))+
  aes(x = x)+
  stat_function(fun = k_left, geom = "line", size = 1, color = "green")+
  stat_function(fun = k_right, geom = "line", size = 1, color = "purple")+
  geom_label(x = 0.8, y = k_left(0.8), label = "Dive Left", color = "green")+
  geom_label(x = 0.8, y = k_right(0.8), label = "Dive Right", color = "purple")+
  ...
  geom_point(x = 0.417, y = 79.57, size = 2)+
  geom_label(x = 0.383, y = 25, label = expression(paste(q^{"*"}==0.417)))+
  scale_x_continuous(breaks = seq(0,1,0.1),
                     limits = c(0,1.05),
                     expand = c(0,0))+
...
- .hi-purple[Mixed Strategy Nash Equilibrium (MSNE)]: (.red[p], .blue[q]) = (.red[0.383], .blue[0.417])
  ...
  - .hi-blue[Goalie]'s expected payoff: .red[20.41]
  - Note they sum to 1 (despite my rounding error)
